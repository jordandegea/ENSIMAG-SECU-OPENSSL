...
OpenSSL : OpenSSL est une boîte à outils de chiffrement comportant deux bibliothèques (libcrypto fournit les algorithmes cryptographiques, libssl implémente le protocole Transport Layer Security (TLS) ainsi que son prédécesseur Secure Sockets Layer (SSL)) et une interface en ligne de commande (openssl). \\
payload : En français "Charge Utile". C'est le message utile envoyé (seulement la partie exclue de tout header ou méta-données utilisées par le protocole. \\
exploit : élément de programme permettant à un individu ou à un logiciel malveillant d'exploiter une faille de sécurité informatique \\
